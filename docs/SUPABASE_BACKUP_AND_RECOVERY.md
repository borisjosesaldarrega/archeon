...
- A logical backup created at `2026-08-22T03:34:05Z` passed structural verification: `roles.sql` 189 bytes, `schema.sql` 29,768 bytes, and `data.sql` 58,232 bytes. Its Git-ignored `manifest.json` records SHA-256 hashes and successful checks for all expected tables, policies, grants, views, functions, triggers, sanitized roles, and `COPY` data statements.
